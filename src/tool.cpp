...
bool Tool::show_json(std::string& out, const nlohmann::json& args, int depth) {
    ...
    if (args.is_object()) {
        ...
        out += ('[' + std::to_string(size) + " items]\n");
        for (const auto& item : args) {
            if (item.is_object()) {
                show_json(out, item, depth + 1);
            }
            else {
                out += (indent + "- ");
                show_json(out, item, depth + 2);
                out += "\n";
            }
            out += "\n";
        }
...
std::string Tool::show_json_text(const std::string& json_args) {
    std::string out = "";
    try {
        auto args = nlohmann::json::parse(json_args);
        if (!show_json(out, args, 0)) {
            out = json_args + "\n";
        }
    } catch (...) {
        out = json_args + "\n";
...
void Tool::show_arguments(const std::string& json_args) {
    LOG_INFO(u8"🔧Tool", name() + " [arguments]");
    TOUT::append(TUI::AnsiColor_Bright_Black, show_json_text(json_args));
}
